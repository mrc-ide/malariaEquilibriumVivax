                                                   ...
                                                   malariasimulationoutput = F)
{
  ...
  if(p$enable_heterogeneity == F){p$n_heterogeneity_groups <- 1}
  ...
  if(is.null(do_not_integrate)){
    ...
    T_eq <- array(Z[,,iT+kk_vec*6], dim = c(na,nh,nk))
    P_eq <- array(Z[,,iP+kk_vec*6], dim = c(na,nh,nk))
    S_eq <- array(Z[,,iS+kk_vec*6], dim = c(na,nh,nk))
    I_PCR_eq <- array(Z[,,iI_PCR+kk_vec*6], dim = c(na,nh,nk))
    I_LM_eq <- array(Z[,,iI_LM+kk_vec*6], dim = c(na,nh,nk))
    I_D_eq <- array(Z[,,iI_D+kk_vec*6], dim = c(na,nh,nk))
