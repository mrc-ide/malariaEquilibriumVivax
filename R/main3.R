...
vivax_equilibrium_init_create <- function(age, ft,
                                          EIR, p, K_max = 30, MW_age_rates_prop = NULL, use_mid_ages = NULL, divide_omega = F, malariasimulationoutput = F)
{
  ...
  if(!isTRUE(MW_age_rates_prop)){
    ...
    age_mid_point[na] <- 0.5 * (age[na] + age[na+1])
  ...
  if(!isTRUE(use_mid_ages)){
    ...
    for (i in 1:na)
    {
      foi_age[i] <- 1 - (p$rho * exp(-age_mid_point/p$a0))  #force of infection for each age group
    }
  }
  ...
  if(isTRUE(malariasimulationoutput)){
    ...
    browser()
    sum(unlist(lapply(1:p$n_heterogeneity_groups, function(het){sum(malsimres[[het]][,"S"])})))
